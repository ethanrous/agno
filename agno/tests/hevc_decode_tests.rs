...
fn dogs_heic_dump_raw_rgb() {
    ...
    // Tile 0
    let pic0 = decode_hevc_still(&heif.hvcc, &heif.tiles[0]).unwrap();
    let y0_bytes: Vec<u8> = pic0.y_plane().iter().map(|&v| v as u8).collect();
    std::fs::write("/tmp/dogs_tile0_y.raw", &y0_bytes).unwrap();
    eprintln!("Dumped tile 0 Y: {}x{}", pic0.width, pic0.height);
    ...
    // Extract tile 10 as Annex B for libde265 comparison
    let hvcc = &heif.hvcc;
    let tile = &heif.tiles[10];
    let start_code: &[u8] = &[0x00, 0x00, 0x00, 0x01];
    let mut out = Vec::new();
    let num_arrays = hvcc[22] as usize;
    let mut pos = 23;
    ...
    eprintln!("Wrote tile 10 Annex B: {} bytes", out.len());
}
...
fn dogs_heic_tile_diagnostics() {
    ...
    // Extract tile 0 for reference comparison
    extract_tile0_annex_b(&test_data("dogs.heic"), "/tmp/dogs-tile0.265");

    // Decode tile 0 and check per-CTU-row quality
    let pic = decode_hevc_still(&heif.hvcc, &heif.tiles[0]).unwrap();
    eprintln!("  tile 0: {}x{}", pic.width, pic.height);

    // Check if reference exists (generate with: ~/repos/external/libde265/build/dec265/dec265 /tmp/dogs-tile0.265 -o /tmp/dogs-tile0-ref.yuv)
    let ref_path = "/tmp/dogs-tile0-ref.yuv";
    if std::path::Path::new(ref_path).exists() {
        let (ref_y, _, _) = load_yuv420p_reference(ref_path, pic.width, pic.height);
        let y_psnr = plane_psnr_i16(pic.y_plane(), &ref_y, 8);
        eprintln!("  tile 0 Y PSNR: {:.2} dB", y_psnr);
        // Per-CTU PSNR for rows 0-2 to locate first divergence
        let ctb_cols = (pic.width + 31) / 32;
        for row in 0..3u32 {
        ...
        let (_, ref_cb, ref_cr) = load_yuv420p_reference(ref_path, pic.width, pic.height);
        let cb_psnr = plane_psnr_i16(pic.cb_plane(), &ref_cb, 8);
        let cr_psnr = plane_psnr_i16(pic.cr_plane(), &ref_cr, 8);
        eprintln!("  tile 0 Cb PSNR: {:.2} dB", cb_psnr);
        eprintln!("  tile 0 Cr PSNR: {:.2} dB", cr_psnr);
        // Find worst chroma errors
    ...
    // Also check a few more tiles
    for i in [0, 1, 8, 16, 24, 40] {
        if i < heif.tiles.len() {
