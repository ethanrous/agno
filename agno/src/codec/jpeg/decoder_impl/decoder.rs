...
impl<R: Read> Decoder<R> {
    ...
    /// Colour transform to use when decoding the image.
    ...
    pub fn scale(&mut self, requested_width: u16, requested_height: u16) -> Result<(u16, u16)> {
        ...
        let idct_size = super::idct::choose_idct_size(
            frame.image_size,
            Dimensions {
...
fn color_convert_line_ycbcr(data: &[Vec<u8>], output: &mut [u8]) {
    ...
    let arch_specific_pixels = {
        if let Some(ycbcr) = super::arch::get_color_convert_line_ycbcr() {
            #[allow(unsafe_code)]
            unsafe {
